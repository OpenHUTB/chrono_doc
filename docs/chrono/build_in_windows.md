...
1. 克隆仓库
    ...
    ```
2. 打开 cmake-gui.exe 配置 chrono

    设置好`Where is the source code`为 `chrono`；`Where to build the binarys:`为`chrono\build`

    依此点击`Configure`按钮，选择合适的平台，比如`Visual Studio 17 2022`。

    点击`Generate`。

!!! 注意
   如果点击`Configure`报没有 Eigen3 的错误（ERROREigen3 cannot be found），则运行`chrono\contrib\build-scripts\windows\buildEigen.bat`进行Eigen安装。然后再点击`Configure`和`Generate`。

3. 使用 VS2019/2022 双击打开`chrono\build\Chrono.sln`。选择`Release`或`Debug`进行构建。
